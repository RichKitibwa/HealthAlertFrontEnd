...
class AmbulanceIncomingDispatchScreen extends StatefulWidget {
  ...
  const AmbulanceIncomingDispatchScreen({Key? key, required this.caseId})
    : super(key: key);

  @override
  State<AmbulanceIncomingDispatchScreen> createState() =>
      _AmbulanceIncomingDispatchScreenState();
}

class _AmbulanceIncomingDispatchScreenState
    extends State<AmbulanceIncomingDispatchScreen> {
  ...
  Future<void> _acceptDispatch() async {
    ...
    try {
      final callable = FirebaseFunctions.instance.httpsCallable(
        'acceptAmbulanceDispatch',
      );
      final result = await callable.call({'caseId': widget.caseId});
      ...
      if (mounted) {
        if (success ||
          ...
          Navigator.pushReplacement(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  AmbulanceEnRouteScreen(caseId: widget.caseId),
            ),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                response['message']?.toString() ??
                    AppLocalizations.of(context)!.errorAcceptingDispatch,
              ),
              backgroundColor: Colors.red,
            ),
          );
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.errorAcceptingDispatch),
            backgroundColor: Colors.red,
          ),
        );
      }
                ...
                children: [
                          ...
                          children: [
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        l10n.reviewDetailsAcceptProceed,
                                        style: TextStyle(
                                          fontFamily: 'Inter',
                            ...
                            if (patientName.isNotEmpty) ...[
                              _InlineInfoRow(
                                ...
                                label: l10n.patientLabel,
                                value:
                                    '$patientName${patientDesc.isNotEmpty ? ' ($patientDesc)' : ''}',
                              ),
                              const SizedBox(height: 10),
                            ],
                            _InlineInfoRow(
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: (_isAccepting || !canAccept)
                          ? null
                          : _acceptDispatch,
